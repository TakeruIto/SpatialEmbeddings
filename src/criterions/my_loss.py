...
class SpatialEmbLoss(nn.Module):
    ...
    def forward(self, prediction, instances, labels, w_inst=1, w_var=10, w_seed=1, iou=False, iou_meter=None):
        w_class = np.array([4.88790299, 9.25677919, 1.65649274, 8.31045215, 8.52187166, 8.5372937, 9.55908201, 7.47551424])
   #     w_class = np.array([self.foreground_weight for _ in range(8)])
        ...
        for b in range(0, batch_size):
            ...
            for cls in range(2+self.n_sigma,prediction[b].shape[0]):
                
                seed_map = torch.sigmoid(
                    prediction[b, cls]).unsqueeze(0)  # 1 x h x w
                # loss accumulators
                var_loss = 0
                instance_loss = 0
                seed_loss = 0
                ...
                for id in instance_ids:
                    in_mask = instance.eq(id)*(label==(cls-3)) # 1 x h x w
                    if in_mask.sum()>0: 
                        ...
                        # seed loss
                        seed_loss += w_class[cls-(2+self.n_sigma)] * torch.sum(
