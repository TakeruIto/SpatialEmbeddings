...
dataset_it = torch.utils.data.DataLoader(
    dataset, batch_size=1, shuffle=True, drop_last=False, num_workers=4, pin_memory=True if args['cuda'] else False)
itr = iter(dataset_it)
# load model
...
with torch.no_grad():
    ...
    if args['display']:
        ...
        for i in range(4,len(output[0])):
            seed = torch.sigmoid(output[0][i]).cpu()
            visualizer.display(seed, 'seed', i)
